...
class MealModel:
    ...
    def update_name(self):
        ...
        cur = con.cursor(cursor_factory=RealDictCursor)
        try:
            cur.execute("""UPDATE meals SET meal_name='{}', price='{}' WHERE
             meal_id='{}'""".format(self.meal_name, self.price, self.meal_id))
            con.commit()
        except psycopg2.DatabaseError as e:
            return {'message': '{}'.format(e)}
        con.close()
    ...
    def save(self):
        ...
        try:
            query = """insert into meals (meal_name, price) 
            values('{}','{}')""".format(self.meal_name, self.price)
